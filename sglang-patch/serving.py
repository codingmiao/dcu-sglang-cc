...
def _cached_prompt_tokens(usage) -> int:
    """sglang --enable-cache-report 时，usage.prompt_tokens_details.cached_tokens 是命中 token 数。"""
...
def _anthropic_usage(
    ...
    *,
    ...
    """把 OpenAI UsageInfo 转成 AnthropicUsage。

    与 sglang >= 0.5.14 的语义一致：input_tokens 是"未命中部分"
    （prompt - cached），命中部分单独放 cache_read_input_tokens，
    ...
    if include_input:
        ...
        fields["input_tokens"] = max(prompt - cached, 0)
        if cached:
...
class AnthropicServing:
    ...
    async def _generate_anthropic_stream(
        ...
        async for sse_line in openai_stream:
            ...
            # Usage chunk：捕获完整 usage（含缓存命中），供 message_delta 使用
            ...
            # Handle reasoning content deltas（--reasoning-parser 解析出的思考内容）
            if delta.reasoning_content:
                if not content_block_open or content_block_type != "thinking":
                    if content_block_open:
                        stop_event = AnthropicStreamEvent(
                            type="content_block_stop",
                            index=content_block_index,
            ...
            if delta.content is not None and delta.content != "":
                ...
                if not content_block_open or content_block_type != "text":
                    if content_block_open:
                        stop_event = AnthropicStreamEvent(
                            type="content_block_stop",
                            index=content_block_index,
    ...
    def _convert_response(
        ...
        # Add reasoning content as a thinking block（--reasoning-parser 解析出的思考内容）
        if choice.message.reasoning_content:
            content.append(
                AnthropicContentBlock(type="thinking", thinking=choice.message.reasoning_content)
            )
